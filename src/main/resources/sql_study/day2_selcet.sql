...
select
	* -- 컬럼이름들, *은 모든 컬럼의미
from
	product; -- 테이블 or 가상테이블alter
    
-- as(별칭) 결과 컬럼 이름을 바꿀 수 있음.
select
	producu_name as productName
from
	product;
...
-- price >= 3000 && price <= 4000 와 가은 동작
select * from product where create_at between 
'2026-05-19 00:00:00' and '2026-05-20 00:00:00';

-- in 연산 "이중에 하나라도 맞으면" (or의 단축형)
select * from product where product_name in ('크로와상','베이글');

-- like 연산 "패턴매칭"
select * from product where product_name like '%라떼%'; -- 라떼가 들어간 모든 것
select * from product where product_name like '카%'; -- 카로 시작하는 모든것product
-- b-tree 구조때문에 ~로 시작하는 검색이 빠르다.
select * from product where product_name like '__쿠키'; -- 정확히 2글자+'쿠키' 인것
...
-- limit n offset m: m개 건너뛰고 위에서 n개 보여줘
-- 페이지네이션에서 한번에 20개 게시글을 보여줄때
-- limit 20 offset m * (page - 1)
...
-- 2) content가 null이 아닌 게시글들 중에 오래된 순으로 3개만 조회해주세요.
select *
from post
order by create_at asc
limit 3;

-- 3) post에서 가장 최신순으로 제목만 3개만 조회하고, 컬럼이름을 'title'으로 조회
select title as '제목'
from post
