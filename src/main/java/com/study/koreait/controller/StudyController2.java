...
public class StudyController2 {
    ...
    public Integer add(Integer n1, Integer n2) {
        log.info("add 수신완료");
        log.info("들어온 데이터 {},{}", n1, n2);
        return n1 + n2;
    }

    @GetMapping("/avg")
    public Integer avg(Integer n1, Integer n2 , Integer n3) {
        log.info("avg 수신완료");
        log.info("들어온 데이터 {},{},{}", n1,n2,n3);
        int sum = n1+n2+n3;
        return sum/3;
    }
    ...
    public ResponseEntity<?> getProductById(@PathVariable int id) {
        List<ProductResDto> dtos = List.of(
                new ProductResDto(1, "아이패드", 8700000),
                new ProductResDto(2, "로지텍 마우스", 670000),
                new ProductResDto(3, "무접점 키보드", 200000)
        );
        ...
        // 옵셔녈.orElseThrow()
        // 커스텀 에러를 던져줘야함.
        ProductResDto dto = optDto.orElseThrow(() -> new ProductException("해당 id는 존재하지 않습니다.", HttpStatus.NOT_FOUND));
    ...
    public Map<String, Object> getProductsByPriceFilter(@RequestParam int minPrice, @RequestParam int maxPrice) {
        List<ProductResDto> dtos = List.of(
                new ProductResDto(1, "아이패드", 8700000),
                new ProductResDto(2, "로지텍 마우스", 670000),
                new ProductResDto(3, "무접점 키보드", 200000)
    ...
    @PostMapping("/product")
    public ResponseEntity<?> addProduct(@RequestBody AddProductReqDto dto) {
        // 상품등록
